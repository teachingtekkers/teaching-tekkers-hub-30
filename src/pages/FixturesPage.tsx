...
const FixturesPage = () => {
    ...
    for (let i = 0; i < teamsList.length; i++) {
      for (let j = i + 1; j < teamsList.length; j++) {
        matches.push({
          id: `m-${Date.now()}-${order}`,
          fixture_set_id: "",
          round_name: `Round ${Math.ceil(order / Math.floor(teamsList.length / 2))}`,
          kickoff_order: order,
  ...
  const generateKnockoutMatches = (teamsList: FixtureTeam[]): FixtureMatch[] => {
    ...
    for (let i = 0; i < teamsList.length; i += 2) {
      matches.push({
        ...
        kickoff_order: order,
        home_team_id: teamsList[i]?.id || null,
        away_team_id: teamsList[i + 1]?.id || null,
        home_score: null,
        away_score: null,
        created_at: new Date().toISOString(),
      });
  ...
  const handleGenerate = () => {
    if (!selectedCampId || !fixtureSetName || teams.some(t => !t.name.trim())) {
      toast.error("Please fill in all fields and team names");
      return;
    }
    ...
    if (format === "group_stage") {
      ...
      // group + knockout: group stage first, then semis/final placeholders
      matches = [
        ...generateGroupStageMatches(fixtureTeams),
        {
          id: `m-${Date.now()}-sf1`, fixture_set_id: setId, round_name: "Semi Final", kickoff_order: 100,
          home_team_id: null, away_team_id: null, home_score: null, away_score: null, created_at: new Date().toISOString(),
        },
        {
          id: `m-${Date.now()}-sf2`, fixture_set_id: setId, round_name: "Semi Final", kickoff_order: 101,
          home_team_id: null, away_team_id: null, home_score: null, away_score: null, created_at: new Date().toISOString(),
        },
        {
          id: `m-${Date.now()}-f`, fixture_set_id: setId, round_name: "Final", kickoff_order: 102,
          home_team_id: null, away_team_id: null, home_score: null, away_score: null, created_at: new Date().toISOString(),
        },
      ];
    }
  ...
  const getTeamName = (set: FixtureSet, teamId: string | null) => {
    if (!teamId) return "TBD";
    return set.teams.find(t => t.id === teamId)?.name || "TBD";
  };

  const getTeamColour = (set: FixtureSet, teamId: string | null) => {
    if (!teamId) return "#94a3b8";
    return set.teams.find(t => t.id === teamId)?.colour || "#94a3b8";
  };
    ...
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          ...
          <p className="text-muted-foreground">Generate and manage camp fixtures</p>
        </div>
            ...
            <div className="space-y-4 pt-2">
              <div className="space-y-2">
                ...
                <Select value={selectedCampId} onValueChange={setSelectedCampId}>
                  ...
                  <SelectContent>
                    {mockCamps.map(c => <SelectItem key={c.id} value={c.id}>{c.name} — {c.club_name}</SelectItem>)}
                  </SelectContent>
                </Select>
              </div>
              <div className="space-y-2">
      ...
      {fixtureSets.length === 0 ? (
        <Card><CardContent className="py-12 text-center"><p className="text-muted-foreground">No fixtures created yet. Click "Create Fixtures" to generate a tournament.</p></CardContent></Card>
      ) : (
        <div className="space-y-4">
          {fixtureSets.map(set => {
            ...
            return (
              <Card key={set.id}>
                <CardHeader className="pb-3 cursor-pointer" onClick={() => setExpandedSet(isExpanded ? null : set.id)}>
                ...
                {isExpanded && (
                  <CardContent className="space-y-4">
                    {rounds.map(round => (
                      <div key={round}>
                        <h4 className="font-semibold text-sm mb-2">{round}</h4>
                        <div className="space-y-2">
