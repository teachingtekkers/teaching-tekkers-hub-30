...
const EquipmentPage = () => {
  ...
  const handleAddItem = () => {
    ...
    const newItem: EquipmentItem = {
      ...
      total_quantity: itemForm.total_quantity,
  ...
  const handleAssign = () => {
    ...
    const newAssignment: EquipmentAssignment = {
      ...
      equipment_item_id: assignForm.equipment_item_id,
      camp_id: assignForm.camp_id || null,
      coach_id: assignForm.coach_id || null,
      quantity_out: assignForm.quantity_out,
      quantity_returned: 0,
    ...
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          ...
          <p className="text-muted-foreground">Track and assign equipment for camps</p>
        </div>
      ...
      <div className="grid gap-4 sm:grid-cols-3">
        ...
        <Card>
          <CardContent className="pt-6">
            ...
            <p className="text-2xl font-bold">{assignments.filter(a => a.quantity_returned < a.quantity_out).length}</p>
          </CardContent>
        </Card>
        <Card>
          <CardContent className="pt-6">
            ...
            <p className="text-2xl font-bold flex items-center gap-1">
              {totalMissing}
              {totalMissing > 0 && <AlertTriangle className="h-4 w-4 text-[hsl(var(--warning))]" />}
            </p>
          </CardContent>
        </Card>
      </div>
                ...
                <Card key={item.id}>
                  <CardContent className="p-4 space-y-2">
                    ...
                    <div className="grid grid-cols-3 gap-2 text-sm text-center">
                      <div><span className="text-muted-foreground block">Out</span><strong>{stats.totalOut}</strong></div>
                      <div><span className="text-muted-foreground block">Returned</span><strong>{stats.totalReturned}</strong></div>
                      <div>
                ...
                <Card key={a.id}>
                  <CardContent className="p-4 space-y-2">
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      {missing === 0 && <Badge className="bg-[hsl(var(--success))] text-[hsl(var(--success-foreground))]">Complete</Badge>}
                    </div>
                  </CardContent>
                </Card>
              );
