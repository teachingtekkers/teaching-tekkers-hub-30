...
const PayrollPage = () => {
  ...
  const handleGenerate = () => {
    ...
    if (existing.length > 0) {
      toast.error("Payroll already generated for this week");
      return;
    }
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid gap-4 sm:grid-cols-3">
        ...
        <Card>
          <CardContent className="pt-6">
            <p className="text-sm text-muted-foreground">Coaches Paid</p>
            <p className="text-2xl font-bold">{coachSummary.length}</p>
          </CardContent>
        </Card>
        <Card>
          <CardContent className="pt-6">
            <p className="text-sm text-muted-foreground">Camps Covered</p>
            <p className="text-2xl font-bold">{weekCamps.length}</p>
          </CardContent>
        </Card>
      </div>

      {weekPayroll.length === 0 ? (
        <Card><CardContent className="py-12 text-center"><p className="text-muted-foreground">No payroll generated for this week. Click "Generate" to create payroll from coach assignments.</p></CardContent></Card>
      ) : (
            ...
            {weekCamps.map(camp => {
              ...
              return (
                <Card key={camp.id}>
                  <CardHeader className="pb-2">
                    <div className="flex items-center justify-between">
                      <CardTitle className="text-base">{camp.name} — {camp.club_name}</CardTitle>
                      <Badge variant="secondary">€{campTotal.toFixed(2)}</Badge>
                    </div>
                  </CardHeader>
                  <CardContent>
                    <Table>
