...
const InvoicesPage = () => {
  ...
  const handleGenerate = () => {
    ...
    if (existing) {
      toast.error("Invoice already exists for this camp");
      return;
    }
  ...
  const statusBadge = (status: ClubInvoice["status"]) => {
    switch (status) {
      ...
      case "sent":
        return <Badge className="bg-[hsl(var(--warning))] text-[hsl(var(--warning-foreground))]">Sent</Badge>;
      case "paid":
        return <Badge className="bg-[hsl(var(--success))] text-[hsl(var(--success-foreground))]">Paid</Badge>;
    }
  };
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          <h1 className="text-2xl font-bold">Club Invoices</h1>
          <p className="text-muted-foreground">Generate and track club payments based on attendance</p>
        </div>
      ...
      <div className="grid gap-4 sm:grid-cols-3">
        <Card>
          <CardContent className="pt-6">
            ...
            <p className="text-2xl font-bold">€{invoices.reduce((s, i) => s + getEffectiveAmount(i), 0).toFixed(2)}</p>
          </CardContent>
        </Card>
        <Card>
          <CardContent className="pt-6">
            <p className="text-sm text-muted-foreground">Paid</p>
            <p className="text-2xl font-bold">€{invoices.filter(i => i.status === "paid").reduce((s, i) => s + getEffectiveAmount(i), 0).toFixed(2)}</p>
          </CardContent>
        </Card>
        <Card>
          <CardContent className="pt-6">
            ...
            <Card key={invoice.id}>
              <CardContent className="p-4 space-y-3">
                ...
                <div className="grid grid-cols-2 gap-2 text-sm">
                  ...
                  <div><span className="text-muted-foreground">Total:</span> <strong>€{getEffectiveAmount(invoice).toFixed(2)}</strong></div>
                </div>
                  ...
                  <TableRow key={invoice.id}>
                    ...
                    <TableCell>
                      <Input
                        ...
                        className="w-24 h-8"
                        placeholder="—"
                        value={invoice.manual_amount ?? ""}
                        onChange={(e) => updateManualAmount(invoice.id, e.target.value)}
                      />
                    </TableCell>
