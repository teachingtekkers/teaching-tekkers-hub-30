...
const ProposalsPage = () => {
  ...
  const [form, setForm] = useState({
    club_name: "", proposal_title: "", proposed_dates: "",
    camp_description: "", price_details: "", notes: "",
  });

  const handleCreate = () => {
    if (!form.club_name.trim() || !form.proposal_title.trim()) {
      toast.error("Club name and title are required");
      return;
    }
    const newProposal: Proposal = {
      ...
      proposed_dates: form.proposed_dates,
      camp_description: form.camp_description || null,
  ...
  const generatePDF = (proposal: Proposal) => {
    ...
    doc.text("Football Coaching Company", 20, 32);
    ...
    // Date
    doc.setFontSize(10);
    doc.text(`Date: ${new Date(proposal.created_at).toLocaleDateString()}`, 20, 82);
    ...
    doc.setFontSize(13);
    doc.setFont("helvetica", "bold");
    doc.text("Proposed Dates", 20, y);
    y += 8;
    doc.setFontSize(11);
    doc.setFont("helvetica", "normal");
    doc.text(proposal.proposed_dates, 20, y);
    y += 16;
    ...
    if (proposal.camp_description) {
      doc.setFontSize(13);
      doc.setFont("helvetica", "bold");
      doc.text("Camp Description", 20, y);
      y += 8;
      doc.setFontSize(11);
      doc.setFont("helvetica", "normal");
      const descLines = doc.splitTextToSize(proposal.camp_description, pageWidth - 40);
    ...
    if (proposal.price_details) {
      ...
      doc.setFontSize(11);
      doc.setFont("helvetica", "normal");
      const priceLines = doc.splitTextToSize(proposal.price_details, pageWidth - 40);
      doc.text(priceLines, 20, y);
      y += priceLines.length * 6 + 10;
    }

    // Footer
    const footerY = doc.internal.pageSize.getHeight() - 20;
    doc.setDrawColor(14, 93, 196);
    ...
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          ...
          <p className="text-muted-foreground">Generate and track club camp proposals</p>
        </div>
          ...
          <DialogContent className="max-w-lg max-h-[85vh] overflow-y-auto">
            ...
            <div className="space-y-4 pt-2">
              <div className="space-y-2">
                ...
                <Input value={form.club_name} onChange={e => setForm({ ...form, club_name: e.target.value })} placeholder="e.g. Portmarnock AFC" required />
              </div>
              <div className="space-y-2">
                <Label>Proposal Title</Label>
                <Input value={form.proposal_title} onChange={e => setForm({ ...form, proposal_title: e.target.value })} placeholder="e.g. Summer Camp 2026" required />
              </div>
              <div className="space-y-2">
                <Label>Proposed Dates</Label>
                <Input value={form.proposed_dates} onChange={e => setForm({ ...form, proposed_dates: e.target.value })} placeholder="e.g. 29 Jun – 3 Jul 2026" />
              </div>
              <div className="space-y-2">
                <Label>Camp Description</Label>
                <Textarea value={form.camp_description} onChange={e => setForm({ ...form, camp_description: e.target.value })} rows={3} placeholder="Describe the camp offering..." />
              </div>
              <div className="space-y-2">
                <Label>Price Details</Label>
                <Textarea value={form.price_details} onChange={e => setForm({ ...form, price_details: e.target.value })} rows={2} placeholder="e.g. €140 per child. Club receives €15 per child." />
              </div>
              <div className="space-y-2">
                <Label>Notes</Label>
                <Input value={form.notes} onChange={e => setForm({ ...form, notes: e.target.value })} placeholder="Internal notes (optional)" />
              </div>
        ...
        <DialogContent className="max-w-lg">
          {viewProposal && (
            <>
              ...
              <div className="space-y-4 pt-2">
                <div><p className="text-sm font-medium">Proposed Dates</p><p className="text-sm text-muted-foreground">{viewProposal.proposed_dates}</p></div>
                {viewProposal.camp_description && <div><p className="text-sm font-medium">Description</p><p className="text-sm text-muted-foreground">{viewProposal.camp_description}</p></div>}
                {viewProposal.price_details && <div><p className="text-sm font-medium">Pricing</p><p className="text-sm text-muted-foreground">{viewProposal.price_details}</p></div>}
                {viewProposal.notes && <div><p className="text-sm font-medium">Notes</p><p className="text-sm text-muted-foreground">{viewProposal.notes}</p></div>}
                <Button onClick={() => generatePDF(viewProposal)} className="w-full"><FileDown className="mr-2 h-4 w-4" />Download PDF</Button>
              </div>
            </>
          )}
        </DialogContent>
